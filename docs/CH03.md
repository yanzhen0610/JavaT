...
addition
------
>  撰寫 Java 程式通常都是由定義「類別」開始，宣告方式如下:
>
>[存取修飾字] + 宣告類別 + 類別名稱 + {類別的內容與本體}
>
>存取修飾字(Modifier)是用來宣告可被存取的權限，共有4種，依序為：private, default(又稱無修飾字), protected 以及 public 。private 需同一個class才能存取，default 同一個package下的classes才能存取，protected 也是同一個package下的classes才能存取，不同package下的classes要有繼承關係才能存取，public 則沒有限制存取。
>"class" 是 Java 用來定義類別的關鍵字，範例3.1中類別的名稱是 HelloJava，這與您編輯的檔案（HelloJava.java）主檔名必須相同
>在定義類別名稱時，建議將類別首字母大寫，並在類別名稱上表明類別的作用。在編寫 Java 程式時，一個檔案中可撰寫數個類別，但是只能有一個公開（public）類別，而且檔案主檔名必須與這個公開類別的名稱相同。
...
addition
------
>Java的輸出很簡單，只要一句print()或是println()就可以進行文字輸出了，如果有數字，也可以使用'+'把文字和數字一起輸出。
```java
System.out.println("現在是" + 9 + "月");
```
>這樣就把3段東西合併在一起輸出了，是不是很簡單?
>但是這樣的方便的設計，當然也會衍生出幾個問題。
```java
System.out.println("現在倫敦" + 2 + "點，台北" + 2 + 8 + "點");
```
>這樣輸出就會變成
>現在倫敦2點，台北28點
>在這個過程中，"現在倫敦"是文字，後面如果加了數字2，會先把數字轉成文字，再繼續往後處理。"點，台北"後面加了數字2，會先把數字2和前面的文字合併，再繼續對數字8做合併，從而變成28。
>解決這個問題的方法如下
```java
System.out.println("現在倫敦" + 2 + "點，台北" + (2 + 8) + "點");
```
>加了小括號之後，括號內的東西會優先做處理，2+8會先處理成10。
>但還有一個問題
```java
System.out.println(02 + 27208889 + "是台北市政府的電話");
```
>輸出就變成
>27208891是台北市政府的電話，02和27208889先做了一次數學加法運算，在和後面的文字做結合。
>改成
```java
System.out.println("" + 02 + 27208889 + "是台北市政府的電話");
```
...
>
>在 J2SE 5.0 中，您可以
使用 java.util.Scanner 類別取得使用者的輸入，java.util 指的是套件（package）層級，java.util.Scanner 表示 Scanner 這個類別是位於 java/util 這樣的階層之下，現階段您可以將這個階層想像為類似檔案管理的目錄階層。
...
>通常，常量命名會使用全大寫的方式，例如：
```java
final int A_CONSTANT_VARIABLE = 10;
```
>且若如果是方法中的常量需在聲明時即賦值(initialize，及初始化)。若是成員常量則需在構造式(constructor，之後會提到)中完成初始化。
...
Java 中的位元運算是逐位元運算的，例如 10010001 與 01000001 作 AND 運算，是一個一個位元對應運算，答案就是 00000001；
...
而補數運算是將所有的位元 0 變 1，1 變 0，例如 00000001 經補數運算就會變為 11111110，例如下面這個程式所示：
